...
func buildTaskWorkflow(b *strings.Builder, p CodingParams) {
	...
	for _, g := range groups {
		if g.parallel {
			...
			b.WriteString("   **Before dispatching this batch, capture the parent HEAD.** Run ONE Bash call from your parent worktree root FIRST (before spawning any teammate):\n")
			b.WriteString("   ```\n")
			...
			b.WriteString("   **After all teammates return, discover each teammate's branch AND verify it actually committed.** Run ONE Bash call from your parent worktree root:\n")
			b.WriteString("   ```\n")
			...
			b.WriteString("       echo \"ABORT: teammate branch $branch (worktree: $path) still points at PARENT_HEAD $PARENT_HEAD — teammate did not commit in its child worktree.\"\n")
			b.WriteString("       exit 1\n")
			b.WriteString("     fi\n")
			b.WriteString("     echo \"$path => $branch @ $tip\"\n")
			b.WriteString("   done\n")
			b.WriteString("   ```\n")
			b.WriteString("   The Agent tool does NOT return `worktreeBranch` (Claude Code's WorktreeCreate-hook path returns only `worktreePath` — see docs/known-issues.md §3), so discovery via `rev-parse --abbrev-ref HEAD` is authoritative. The additional `$tip = $PARENT_HEAD` guard catches teammates that spawned correctly but bypassed their child worktree (e.g. `cd`-ed to the parent — see docs/known-issues.md §6). If the loop aborts, STOP the batch: post an issue comment naming the misbehaving teammate(s) and their worktree paths, do NOT cherry-pick, do NOT run `git cherry-pick --skip`. Hand off to the user.\n")
			...
			b.WriteString("   ```\n")
			b.WriteString("   Each cherry-pick lands that teammate's single commit on your branch, preserving its original message. **If any cherry-pick fails with a conflict**, immediately run `git cherry-pick --abort`, then STOP: post an issue comment listing the conflicting paths (spec bug — two `[P]` tasks that share a file), and do NOT retry automatically. **Do NOT run `git cherry-pick --skip` under any circumstances** — `--skip` silently drops a teammate's commit. The sanity check above already verified every teammate branch has real work, so any later \"empty commit\" error indicates something is genuinely wrong (spec bug or concurrent edit), not a `--skip`-able state. Surface the error and let the user investigate.\n")
...
func buildTeamDelegation(b *strings.Builder, p CodingParams) {
	...
	b.WriteString("  prompt: <full task context including issue ID, task ID, description, file paths, APPROACH, and commit format>\n")
	b.WriteString("```\n\n")
	b.WriteString("**Important — do NOT embed worktree paths or `cd` instructions in the teammate's `prompt` argument.** Claude Code's `isolation: \"worktree\"` automatically sets the teammate's CWD to the child worktree. If you write phrases like \"work in D:\\...\\worktrees\\<issue>\" or \"cd to the project root\" in the spawn prompt, or embed any absolute path that points at the parent worktree, the teammate may `cd` there and commit in the shared parent worktree instead of its isolated child — this defeats the isolation entirely and will trigger the Task Execution Order sanity check to ABORT the batch (see docs/known-issues.md §6 for the incident that led to this warning). Keep the `prompt` strictly about the task: issue ID, task ID, description, file paths (SCOPE as repo-relative paths like `internal/git/ops.go`, never absolute), APPROACH, commit format. No cwd hints. No absolute paths. No shell commands. The teammate has its own CWD and does not need to know where it is.\n\n")
