...
**Meeting Protocol**: When multiple clarifier agents are launched for the same project, they auto-discover each other via `list_projects` and enter meeting mode — broadcasting user inputs, debating viewpoints, and converging to a single Issue Spec via channel messages. See `agents/clarifier.md` Meeting Protocol section for the full state machine and rules.
