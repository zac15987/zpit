...
Defaults: `coding` / `reviewer` / `clarifier` / `efficiency` = `opus[1m]`; `task_runner` = `sonnet`. History: setting all five to Sonnet previously produced ~90% two-round coding→review cycles — the round-two cost erased the per-token savings. Splitting by role (Opus for judgment, Sonnet for scoped execution) avoids that regression.
