...
**Parallel Commit Protocol** (shared-worktree race mitigation): Parallel `[P]` teammates all commit into the same linked worktree, racing on the shared staging index and on `refs/heads/<branch>.lock`. Each teammate resolves paths via `GIT_DIR=$(git rev-parse --git-dir)` and `GIT_COMMON_DIR=$(git rev-parse --git-common-dir)` (never hard-code `.git/...` — in a linked worktree `.git` is a pointer file, not a directory), seeds its private `GIT_INDEX_FILE="$GIT_DIR/index.zpit.T{N}"` with `git read-tree HEAD` (otherwise commit deletes every unstaged path), uses scoped `git add -- <files>` (pathspec safety net), and wraps `git commit` in a `mkdir "$GIT_COMMON_DIR/zpit-commit.lock"` loop with 5 jittered retries (ref serialization — `git commit` does NOT auto-retry on `cannot lock ref`). The whole sequence runs as a SINGLE Bash tool call because each Bash invocation is a fresh shell and `export` does not persist. Sequential tasks skip the protocol (they own the index). Activation is driven by the `parallel_task_id` line injected by the orchestrator prompt — don't rename/remove that line without updating the agent doc too. **Post-batch main-index resync** (Fix v3, issue #13): each teammate's isolated commit advances HEAD but leaves `$GIT_DIR/index` stale at the pre-batch tree; the orchestrator MUST run `git read-tree HEAD` once after every `[P]` batch (emitted into the Task Execution Order by `buildTaskWorkflow`) so any subsequent sequential task or final-adjustment commit builds on the correct baseline rather than silently reverting the batch. Teammates must NOT resync themselves — concurrent `git read-tree HEAD` on the main index races on `index.lock`. See `docs/known-issues.md` §2 for the three incidents (v1, v2, v3) that shaped this.
