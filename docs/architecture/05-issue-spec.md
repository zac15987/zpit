...
- 平行 teammate 各自擁有一個 child worktree（orchestrator 呼叫 Agent tool 時帶 `isolation: "worktree"`，觸發 zpit 的 `WorktreeCreate` hook 從 orchestrator 的 HEAD 分叉出 `.zpit-children/<slug>`），在自己的 branch 上正常 commit；batch 結束後 orchestrator 把各 teammate 的 branch cherry-pick 回父 branch，再 `git worktree remove` + `git branch -D` 清理。Cherry-pick 衝突（spec bug：兩個 `[P]` task 寫同檔）會被 `cherry-pick --abort` 當面擋下，不會靜默回寫。詳見 `docs/architecture/06-agents.md` §6.3
