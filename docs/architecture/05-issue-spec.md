...
- 平行 teammate 共用 worktree，orchestrator 在 spawn prompt 注入 `parallel_task_id: T{N}` 觸發 **Parallel Commit Protocol**（獨立 `GIT_INDEX_FILE` + scoped `git add --` + `.git/zpit-commit.lock` mkdir 鎖），避免 `.git/index` 與 ref-lock race；詳見 `docs/architecture/06-agents.md` §6.3
