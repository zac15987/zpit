...
- 平行 teammate 共用 worktree，orchestrator 在 spawn prompt 注入 `parallel_task_id: T{N}` 觸發 **Parallel Commit Protocol**（透過 `git rev-parse --git-dir` / `--git-common-dir` 解析路徑、`git read-tree HEAD` seed 私有 `GIT_INDEX_FILE`、scoped `git add --`、`mkdir $GIT_COMMON_DIR/zpit-commit.lock` 序列化 commit），避免共用 staging index 與 ref-lock race；整段序列必須在同一個 Bash tool 呼叫裡跑完。詳見 `docs/architecture/06-agents.md` §6.3
