...
- On Windows, when zpit runs elevated (High IL) and the target app runs unelevated (Medium IL), every UIA call (including `set_value`, `find_element`, `get_ui_tree`) pays a cross-IL mssproxy RPC cost — empirically 100×–400× slower than same-IL. A `set_value` on a file-path field measured 235 ms same-IL but 96 s in the cross-IL session that triggered §8 / §7 of `docs/known-issues.md`. If a UIA write doesn't return within ~5 seconds, fall back to `click_element` on the field + `key`/`type` to enter text. `press_button` is no longer affected (it uses SendInput as of zpit-desktop-mcp 1.2.3).
