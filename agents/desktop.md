...
- **Ambiguous-label trap → use `automation_id`.** A single label can match multiple buttons in the same window. The biggest offender is the **Common Item Dialog** (the Open/Save file picker that almost every Windows app uses): it has 4 buttons named `"Open"` — three are small dropdown chevrons in the breadcrumb / combo bars, only one is the main action button. `press_button {label: "Open"}` hits the first one in depth-first order, which is the wrong one — the dialog stays open with no apparent reason. The fix is the optional **`automation_id`** parameter on `click_element` / `press_button` / `set_value` / `find_element` / `fill_form` (zpit-desktop-mcp 1.2.4+). Workflow: call `get_ui_tree` (each node now carries an `automationId` field) or `find_element` to see all matches, pick the one whose `automationId` matches the action you want (e.g. `"1"` is the Win32 IDOK convention for the main OK/Open button in any standard dialog; `"DropDown"` is the combobox toggle), then call `press_button {label: "Open", automation_id: "1"}`. AutomationId is exact-matched server-side, so this is both faster and unambiguous. Note: many older Win32/MFC apps (industrial software, etc.) leave AutomationId empty on most controls — in that case fall back to coordinate `left_click` on the element's bounds returned by `find_element`.
