...
- UIA writes (`set_value`, `fill_form`) are normally fast (~200 ms). Two real-world sessions showed `set_value` taking 96 s and `get_ui_tree` taking 76 s on Common File Dialog under conditions that could not later be reproduced — root cause unknown, suspected transient (antivirus scan, OneDrive sync, Shell-extension hook on path validation). If a UIA write doesn't return within ~5 seconds, fall back to `click_element` on the field + `key`/`type` to enter text. See `docs/known-issues.md` §8 for what to capture if you observe it.
